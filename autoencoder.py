...
parser.add_argument('--train', type=str, default='True', help='Train the model or not')
args = parser.parse_args()

# Convert the train argument to boolean
train_model = args.train.lower() in ('true', '1', 't', 'y', 'yes')
...
def generate_image_grid(sess, op):
    ...
    """
    x_points = np.arange(-3, 3, 0.5).astype(np.float32)
    y_points = np.arange(-3, 3, 0.5).astype(np.float32)
